...
# Download and unzip Home Assistant config
ZIP_URL="https://github.com/Def-Not-Ollie/CamperPi/raw/main/Home%20Assistant.zip"
TMP_ZIP="/tmp/ha-config.zip"

echo "Downloading Home Assistant config zip..."
curl -L -o "$TMP_ZIP" "$ZIP_URL"

echo "Extracting config..."
rm -rf "$HA_CONFIG_DIR"/*
unzip -o "$TMP_ZIP" -d "$HA_CONFIG_DIR"
